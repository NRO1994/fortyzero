...
class CostModel:
    ...
    def calculate(
        ...
        opex_fixed_annual = self._calculate_opex_fixed(
            opex_config.get("fixed", []), inflation_rate, n_years
        )
        opex_variable_monthly = self._calculate_opex_variable(
            opex_config.get("variable", []), inflation_rate, n_months
        )
    ...
    def _calculate_opex_fixed(
        ...
        for item in fixed_config:
            ...
            escalation = item.get("escalation_rate") or inflation_rate

            for year in range(n_years):
                if indexed:
                    opex[year] += annual_amount * (1 + escalation) ** year
                else:
